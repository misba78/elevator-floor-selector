...
#define FONT_FLOOR_NUM  &lv_font_montserrat_192  // 커스텀 140px 폰트
#define FONT_UNIT       &lv_font_montserrat_36
...
static void update_floor_display(void) {
    ...
    if (g_current_floor <= 17) {
        ...
        ring_color = lv_color_hex(0x0288D1);      // 중층 - 하늘파랑
    } else {
        ring_color = lv_color_hex(0x00BCD4);      // 고층 - 시안
    }
...
static void create_main_screen(void) {
    ...
    lv_obj_set_size(btn_minus, 90, 90);//기존 64, 64
    ...
    lv_obj_set_size(btn_plus, 90, 90); //기존 64, 64
    ...
    lv_obj_align(lbl_hint, LV_ALIGN_CENTER, 0, -160);
...
static void create_confirm_screen(void) {
    ...
    lv_obj_align(lbl_u, LV_ALIGN_CENTER, 130, 0);
